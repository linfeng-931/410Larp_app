...
export default function AccountSetting() {
  ...
  const PasswordsValidate = () => {
    ...
    if (!formData.confirmCurrentPassword)
      newErrors.confirmCurrentPassword = "請輸入目前密碼";
    if (!formData.password) newErrors.password = "請輸入新密碼";
    ...
    } else if (formData.password === formData.confirmCurrentPassword) {
      newErrors.password = "新密碼不得與目前密碼相同";
    }
    ...
    if (PasswordsValidate()) {
      ...
      try {
        await changeUserPassword(
          formData.confirmCurrentPassword,
          formData.password,
        );

        Alert.alert("成功", "密碼更改成功！");
        ...
        if (
          ...
          setErrors((prev) => ({
            ...prev,
            confirmCurrentPassword: "目前密碼輸入錯誤",
          }));
          Alert.alert("驗證失敗", "目前密碼不正確，請重新輸入。");
        } else {
          Alert.alert("密碼更改失敗", "發生未知錯誤，請稍後再試。");
        }
      } finally {
        setLoading(false);
      }
          ...
          <View style={{ width: "100%", paddingHorizontal: 32, gap: 32 }}>
            ...
            <View style={{ gap: 16 }}>
              ...
              <View style={styles.inputFrame}>
                ...
                <Pressable
                  onPress={() =>
                    showconfirmCurrentPassword(!confirmCurrentPassword)
                  }
                >
                  {confirmCurrentPassword ? (
