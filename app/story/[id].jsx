...
import { UsersRound, Shirt, SquareStar, BookType, ClockFading } from "lucide-react-native";
...
export default function AnItem() {
    ...
    const StoryDetail = (
        <View style={{ width: '100%', gap: 16 }}>
            <Text style={[styles.bigTitleNormal, { marginBottom: 8 }]}>{story.title}</Text>
            <View style={styles.cardTagContainer}>
                {story.tag.map((tag, index) => <Text key={index} style={styles.cardTag}>{tag}</Text>)}
            </View>
                ...
                <View style={{ gap: 16 }}>
                    <View style={styles.cardContent}>
                        ...
                        {story.people.length === 1 ?
                            ...
                            <Text style={styles.content1}>{story.people[0]}-{story.people[1]}人</Text>
                        }
                    </View>
                    <View style={styles.cardContent}>
            ...
            <View>
                {story.characters.map((char, index) =>
                    <Character
                        ...
                        tag={char.charTag}
                        styles={styles}
                        color={colorScheme === 'light' ? `${colors.color2}26` : `${colors.color3}26`}
                    />)
                }
            </View>
        </View>
            ...
            >
                ...
                >
                    <View style={styles.main}>
                        ...
                        <View style={{ paddingHorizontal: 32, width: '100%', gap: 60, alignItems: 'center' }}>
                            <Image 
                                source={story.cover} 
                                style={{borderRadius:8, width: 251, height: 316, marginTop: 32}}
                            />
                            {StoryDetail}
                            {StoryContent}
                            {StoryCharactor}
                    ...
                    <StoriesList currentStories={stories} pageStatus={0} styles={styles} colorScheme={colorScheme} />
